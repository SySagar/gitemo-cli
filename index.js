...
async function welcome() {
  const rainbowTitle = chalkAnimation.rainbow("Ready to push the code?\n");
...
async function options() {
  const opts = await inquirer.prompt({
    ...
    type: "list",
    message:
      "Select any of the follwing task you have done in the codebase :\n",
    choices: [
      emoij.feature.task,
      emoij.build.task,
      emoij.fix.task,
...
async function run() {
  if (
    shell.exec(
      `git add . && git commit -m "${messageEmoji} ${messageType} : ${commitMsg}"`
    ).code !== 0
  ) {
