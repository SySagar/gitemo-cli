...
export default (async (gitmojis, options) => {
  ...
  return [{
    ...
    source: (answersSoFor, input) => {
      return Promise.resolve(filtergitemo(input, gitmojis).map(({
        emoji,
        description,
        type
      }) => ({
    ...
    transformer: input => {
      ...
      return `[${length}/${TITLE_MAX_LENGTH_COUNT}]: ${configurationVault.getCapitalizeTitle ? capitaliseTitle(input) : input}`;
    },
    ...(title ? {
      default: title
    } : {})
